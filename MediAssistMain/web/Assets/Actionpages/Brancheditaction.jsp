...
<html>
    ...
    <body>
        ...
        <%
            ...
            String Upd="update tbl_branch set ";
                    ...
                    } else//when it is an image or multipart
                    {
                        ...
                        if (!f_item.getName().equals("")) {
                            if (field_name.equals("txtlogo"))//write the name of image here it is img_product
                            {
                                ...
                                String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Branchownerphoto\\";//path for storing image
                                Random r = new Random();//to generate random number
                                int r_num = r.nextInt(1111) + 999;//values between this are generated

                                photo = "photo_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
...
//writing to server means the folder             
                                } catch (Exception ex) {
                                    out.println(ex);
                                }
                        Upd += "branch_ownerphoto='" + photo + "',";
...
//                if(request.getParameter("txtlogo").equals(""))
//                {

                     Upd += "branch_email='" + value[0] + "',branch_contact='" + value[1] + "',branch_address='" + value[2] + "',branch_ownername='" + value[3] + "',branch_ownercontact='" + value[4]+"',branch_owneremail='" + value[5] + "' where branch_id='" + session.getAttribute("buserid") + "'";

   //System.out.println(Upd);
                   // out.println(Upd);
                    boolean status = obj.executeCommand(Upd);
        ...
        <script type="text/javascript">
            alert("Edit Success..");
            setTimeout(function() {
                window.location.href = '../../Branch/MyProfile.jsp';
            }, 500);//40millisecend it go to next page
        </script>
        <%
            }
